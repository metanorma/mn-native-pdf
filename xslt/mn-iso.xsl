...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format" xmlns:iso="http://riboseinc.com/isoxml" xmlns:mathml="http://www.w3.org/1998/Math/MathML" xmlns:xalan="http://xml.apache.org/xalan" version="1.0">
  ...
  <xsl:variable name="lang-1st-letter" select="''"/>
	...
	<xsl:variable name="isPublished">
		...
		<xsl:choose>
			<xsl:when test="$stage = 'NaN'">false</xsl:when>
		...
		<fo:root xmlns:fo="http://www.w3.org/1999/XSL/Format" font-family="Cambria, FreeSerif, HanSans, NanumGothic, DroidSans" font-size="11pt"> <!-- alternatives: SimSun -->
			...
			<fo:page-sequence master-reference="document" initial-page-number="1" force-page-count="no-force">
				...
				<fo:flow flow-name="xsl-region-body">
					...
					<fo:block-container absolute-position="fixed" left="19mm" top="10mm" height="9mm" width="172mm" border-top="0.5mm solid black" border-bottom="0.5mm solid black" display-align="center">
					...
					<fo:block-container>
						...
						<!-- <fo:block font-size="16pt" font-weight="bold" margin-bottom="18pt">
						 ...
						 -->
						<fo:block font-size="18pt" font-weight="bold" margin-top="6pt" margin-bottom="18pt">
							<fo:block>
								<xsl:value-of select="$title-intro"/>
								<xsl:text> — </xsl:text>
								<xsl:value-of select="$title-main"/>
					...
					<fo:block>
						...
						<fo:block space-before="36pt">&#xA0;</fo:block>
...
<!-- 	<xsl:template match="iso:table" mode="contents">
	...
	<xsl:template match="iso:preferred2" mode="contents">
		<xsl:param name="sectionNum"/>
	...
	<xsl:template match="iso:annex2" mode="contents">
	...
	<xsl:template name="getLevel">
		...
		<xsl:variable name="level">
			<xsl:choose>
				<xsl:when test="ancestor::iso:preface">1</xsl:when>
