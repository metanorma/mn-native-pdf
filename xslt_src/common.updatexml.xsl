...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" 
	...
	<xsl:template name="updateXML">
		...
		<xsl:variable name="updated_xml_step2">
			<xsl:choose>
				<xsl:when test="$namespace = 'ieee' or $namespace = 'iso' or $namespace = 'jis' or $namespace = 'plateau' or $namespace = 'bsi' or $namespace = 'pas'">
					<xsl:if test="$table_if = 'false'">
	...
	<xsl:template match="mn:pagebreak[not(following-sibling::*[1][self::mn:pagebreak])]" mode="update_xml_step_move_pagebreak">	
		...
		<xsl:if test="contains($isLast, 'false')">
			...
			<xsl:if test="(ancestor::mn:preface and following::*[ancestor::mn:preface])
				or (ancestor::mn:sections and following::*[ancestor::mn:sections])
				or (ancestor::mn:annex and following::*[ancestor::mn:annex])"><xsl:text> role="NonStruct"</xsl:text></xsl:if>
	...
	<xsl:if test="$namespace = 'ieee' or $namespace = 'iso' or $namespace = 'jis' or $namespace = 'plateau' or $namespace = 'bsi' or $namespace = 'pas'">
		<!-- =========================================================================== -->
		<!-- STEP2: add 'fn' after 'eref' and 'origin', if referenced to bibitem with 'note' = Withdrawn.' or 'Cancelled and replaced...'  -->
		<!-- =========================================================================== -->
	...
	<xsl:template match="text()[not( (ancestor::mn:bibdata and not(ancestor::mn:title)) or 
				...
				ancestor::mn:name or (ancestor::mn:fmt-name and not(ancestor::mn:semx[@element = 'name'])) or
