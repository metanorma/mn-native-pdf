...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" 
	...
	<xsl:template name="insertRootStyle">
		...
		<xsl:variable name="additional_fonts_">
			<xsl:for-each select="//*[local-name() = 'metanorma'][1]/*[local-name() = 'metanorma-extension']/*[local-name() = 'presentation-metadata'][*[local-name() = 'name'] = 'fonts']/*[local-name() = 'value'] | 
					//*[local-name() = 'metanorma'][1]/*[local-name() = 'presentation-metadata'][*[local-name() = 'name'] = 'fonts']/*[local-name() = 'value']">
						...
						<xsl:otherwise>
							...
							<xsl:choose>
								...
								<xsl:otherwise>
									...
									and not(local-name() = 'thead') and not(local-name() = 'tfoot') and not(local-name() = 'fmt-footnote-container')]" /> <!-- process all table' elements, except name, header, footer, note, source and dl which render separaterely -->
								</xsl:otherwise>
							</xsl:choose>
	...
	<xsl:template match="*[local-name()='table']/*[local-name()='note' or local-name() = 'example'] |
						*[local-name()='table']/*[local-name()='tfoot']//*[local-name()='note' or local-name() = 'example']" priority="2">
		<xsl:choose>
			<xsl:when test="$namespace = 'jis'">
	...
	<xsl:template match="*[local-name()='table']/*[local-name()='note' or local-name()='example']/*[local-name()='p'] |
	*[local-name()='table']/*[local-name()='tfoot']//*[local-name()='note' or local-name()='example']/*[local-name()='p']" priority="2">
		<xsl:apply-templates/>
	</xsl:template>
	...
	<xsl:template name="get_fn_list">
		<xsl:choose>
			...
			<xsl:otherwise>
				...
				<xsl:for-each select="ancestor::*[local-name() = 'metanorma']/*[local-name()='boilerplate']/* | 
					ancestor::*[local-name() = 'metanorma']//*[local-name()='preface']/* |
					ancestor::*[local-name() = 'metanorma']//*[local-name()='sections']/* | 
					ancestor::*[local-name() = 'metanorma']//*[local-name()='annex'] |
					ancestor::*[local-name() = 'metanorma']//*[local-name()='bibliography']/*">
					<xsl:sort select="@displayorder" data-type="number"/>
					<!-- commented:
	...
	<xsl:template match="*[local-name()='fn']">
		...
		<xsl:choose>
			...
			<xsl:when test="not(ancestor::*[local-name() = 'table'][1]/*[local-name() = 'fmt-footnote-container']/*[local-name() = 'fmt-fn-body'][@id = $target]) and
							$footnotes/*[local-name() = 'fmt-fn-body'][@id = $target]">
	...
	<xsl:template match="*[local-name() = 'figure']/*[local-name() = 'fmt-name'] | 
														...
														*[local-name() = 'recommendation']/*[local-name() = 'fmt-name'] |
														*[local-name() = 'requirement']/*[local-name() = 'fmt-name']" mode="contents">		
		<xsl:apply-templates mode="contents"/>
		<xsl:text> </xsl:text>
	</xsl:template>
	...
	<xsl:template match="*[local-name() = 'figure']/*[local-name() = 'fmt-name'] | 
														...
														*[local-name() = 'permission']/*[local-name() = 'fmt-name'] |
														*[local-name() = 'recommendation']/*[local-name() = 'fmt-name'] |
														*[local-name() = 'requirement']/*[local-name() = 'fmt-name'] |
														*[local-name() = 'sourcecode']/*[local-name() = 'fmt-name']" mode="bookmarks">		
		<xsl:apply-templates mode="bookmarks"/>
		<xsl:text> </xsl:text>
	</xsl:template>
	...
	<xsl:template match="*[local-name() = 'requirement'] |
												*[local-name() = 'recommendation'] | 
												*[local-name() = 'permission']" mode="contents" priority="3"/>
	...
	<xsl:template match="*[local-name() = 'requirement'] |
												*[local-name() = 'recommendation'] | 
												*[local-name() = 'permission']" mode="bookmarks" priority="3"/>
...
<!-- ========== -->
	...
	*[local-name() = 'table']/*[local-name() = 'tfoot']//*[local-name() = 'example']/*[local-name() = 'name']">
		<fo:inline xsl:use-attribute-sets="example-name-style">
			<xsl:if test="$namespace = 'jis'">
				<xsl:if test="not($vertical_layout = 'true')">
					<xsl:attribute name="font-family">IPAexGothic</xsl:attribute>
	...
	<xsl:template match="*[local-name() = 'bookmark']" name="bookmark">
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				...
				<fo:inline id="{@id}" font-size="1pt"><xsl:if test="preceding-sibling::node()[local-name() = 'fmt-review-start'][@source = $bookmark_id] and 
						following-sibling::node()[local-name() = 'fmt-review-end'][@source = $bookmark_id]"><xsl:attribute name="line-height">0.1</xsl:attribute></xsl:if><xsl:value-of select="$hair_space"/></fo:inline>
	...
	<xsl:template match="*[local-name() = 'sections']" mode="update_xml_step1">
		<xsl:copy>
			...
			<xsl:for-each select="* | 
				ancestor::*[local-name() = 'metanorma']/*[local-name()='bibliography']/*[local-name()='references'][@normative='true'] |
				ancestor::*[local-name() = 'metanorma']/*[local-name()='bibliography']/*[local-name()='clause'][*[local-name()='references'][@normative='true']]">
				<xsl:sort select="@displayorder" data-type="number"/>
	...
	<xsl:template match="*[local-name() = 'image'][not(.//*[local-name() = 'passthrough'])] | 
